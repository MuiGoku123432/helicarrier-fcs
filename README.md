...
The baseline controller uses `velocityGain=0.80`, `positionGain=0.0225`, `integralGain=0.010`, a 0.45 degree/s command slew, and a 6 degree tilt cap. The slew was raised from 0.15 after drift-test runs 5 and 6; see HANDOFF.md. Do not change its plant sign, gains, protocol, pod validation, or fallback behavior without retaining the run-3 configuration and producing a saved comparison run.
